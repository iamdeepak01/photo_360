...
import com.visticsolution.posterbanao.editor.moviewidget.FilterType;
...
public class RecyclerVideoFilterAdapter extends RecyclerView.Adapter<RecyclerVideoFilterAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(ViewHolder viewHolder, final int position) {
        int i = position;
        Glide.with(this.context).load(this.mItemList.get(i).imgRes).thumbnail(0.1f).apply((((new RequestOptions().dontAnimate()).centerCrop()).placeholder(R.drawable.placeholder)).error(R.drawable.placeholder)).into(viewHolder.imageView);
